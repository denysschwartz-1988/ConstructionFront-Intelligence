# ConstructionFront Project Intelligence Schema v0.2

Schema v0.2 refines update/event milestone classification before full-database migration.

## Controlled `milestoneType`

Allowed values:

- `Development / Planning`
- `Approval / Permitting`
- `FEED / FEL`
- `ECI / Pre-Construction`
- `FID / Financial Close`
- `PPA / Offtake`
- `Procurement Launch`
- `Preferred Bidder / Shortlist`
- `Contract Award`
- `Under Construction`
- `Construction Completed`
- `Operational`
- `M&A`
- `Dispute / Settlement`
- `Financing / Investment`
- `Other`
- `Unknown`

## Classification Rules

Keep `currentProjectStage` separate from `milestoneType`.

- `currentProjectStage` describes where the project currently sits in its lifecycle.
- `milestoneType` describes what the article or update is about.
- `milestoneSummary` explains the event in human-readable detail.

Do not use separate `milestoneType` values for detailed construction states. Map them as follows:

- Construction Started -> `Under Construction`
- Construction Progress -> `Under Construction`
- Substantial Completion -> `Construction Completed`
- Mechanical Completion -> `Construction Completed`
- Practical Completion -> `Construction Completed`
- Testing / Commissioning -> `Construction Completed`, unless the asset is clearly operational
- Commercial Operations -> `Operational`
- Opened to Traffic -> `Operational`

Use `M&A` for acquisitions, divestments, mergers, equity sales, corporate transactions and asset/platform transactions.

Use `FID / Financial Close` for final investment decision, formal project financial close, full notice to proceed and formal project-level financing close.

Use `Financing / Investment` for broader funding, equity investment, corporate finance, JV contribution, debt facility or programme-level investment that is not necessarily a formal project financial close.

Use `PPA / Offtake` for power purchase agreements, LNG offtake, utility purchase agreements and similar commercial offtake events. Do not classify PPA/offtake articles as construction contract awards unless the article separately states a construction contract award.

## Sample Files

The v0.2 clean working sample is sidecar-only:

- `data/project-intelligence/sample/projects_master_sample_5_clean.csv`
- `data/project-intelligence/sample/project_updates_sample_5_clean.csv`
- `data/project-intelligence/sample/sample_schema_validation_report_clean.md`

The original 5-project sample files are preserved for comparison.

## Map Anchors For Programmes

Project master rows may use `mapAnchorType` to distinguish physical locations from display anchors.

Allowed values:

- `Exact Project Location`
- `Approximate Project Location`
- `Corridor Midpoint`
- `Regional Anchor`
- `Country Capital Anchor`
- `Programme / Portfolio Anchor`
- `Unknown`

For national programmes, broad grid expansion programmes, country-level financing records or portfolios without a discrete site, a country capital can be used as a display anchor if the record is clearly labelled approximate.

Required handling:

- `locationPrecision` = `National`
- `mapAnchorType` = `Country Capital Anchor` or `Programme / Portfolio Anchor`
- `mapEligibilityStatus` = `Map Ready - Approximate`
- `mapEligibilityNotes` must say the coordinates are a country-level display anchor only, not the physical project location

Do not use capital-city anchors for discrete projects where the real location is known or can be reasonably identified.

## Monetary Values

Project master rows use display-scale monetary fields:

- `projectValueAmount`
- `projectValueCurrency`
- `projectValueScale`
- `projectValueBasis`
- `projectValueNotes`

Project update/event rows use event-specific value fields:

- `eventValueAmount`
- `eventValueCurrency`
- `eventValueScale`
- `eventValueBasis`
- `eventValueNotes`

`projectValueAmount` and `eventValueAmount` should be readable scaled figures, not raw integers. For example, EUR 177 million should be stored as `177`, `EUR`, `Million`, `Contract Value`. USD 9.75 billion should be stored as `9.75`, `USD`, `Billion`, `Project Financing`.

Allowed `valueScale` values:

- `Thousand`
- `Million`
- `Billion`
- `Unknown`

Allowed `valueBasis` values:

- `Total Project Value`
- `Contract Value`
- `Package Value`
- `Project Financing`
- `Debt Facility`
- `Equity Investment`
- `JV Contribution`
- `PPA / Offtake Value`
- `Settlement Amount`
- `Capex Estimate`
- `Phase Value`
- `Programme Value`
- `Unknown`

Do not convert currencies. Do not infer values that are not supported by the article or source. Notes should explain whether the value is a total project value, package value, contract award, financing close, phase value, settlement, JV contribution or another basis.

## Business Development Fields

`potentialOpportunities` remains a concise narrative field for the commercial reading of the record.

`bdTiming` is now treated as opportunity timing. Use `Supply Chain Opportunity` for records where the main EPC, EPCM, alliance, main contractor or delivery contractor is already selected and the remaining relevance is primarily subcontractor, supplier, consultant, advisory, commissioning, risk or delivery-chain work. `Contractor Route` is retained only as a legacy transition value for older workflow files and should not be used for new sample records.

`opportunityCategories` is a controlled multi-value tag field used for filtering by package, discipline or advisory opportunity. Store multiple values separated by semicolons.

Allowed `opportunityCategories` values:

- `Civil Works`
- `Balance of Plant (BoP)`
- `Electrical BoP`
- `Mechanical BoP`
- `Earthworks`
- `Roadworks / Access Roads`
- `Drainage`
- `Foundations`
- `Piling`
- `Concrete Works`
- `Steel / Structural`
- `Utilities Relocation`
- `Water / Wastewater`
- `Rail Systems`
- `Stations / Buildings`
- `Substations`
- `Transmission Lines`
- `Grid Connection`
- `Cabling`
- `Mechanical Works`
- `Process Plant`
- `Pipelines`
- `Marine / Port Works`
- `Design / Engineering`
- `Owner's Engineer`
- `Environmental / Planning`
- `Project Controls`
- `Commercial Management`
- `Claims / Dispute Support`
- `Legal / Contract Advisory`
- `Insurance / Risk Advisory`
- `Procurement / Supply Chain`
- `Equipment Supply`
- `Commissioning`
- `Operations & Maintenance`
- `Asset Management`
- `Technology / Software`
- `Recruitment / Workforce`
- `Financing / Investment`
- `Other`
- `Unknown`

`targetBuyerTypes` is a controlled multi-value field describing which BD audiences the opportunity may be relevant to. Store multiple values separated by semicolons.

Allowed `targetBuyerTypes` values:

- `Developer / Owner`
- `EPC Contractor`
- `EPCM Contractor`
- `Main Contractor`
- `Civil Contractor`
- `Electrical Contractor`
- `Mechanical Contractor`
- `Specialist Subcontractor`
- `Equipment Supplier`
- `Engineering Consultant`
- `Commercial / Claims Consultant`
- `Legal Advisor`
- `Insurer / Broker`
- `Lender / Investor`
- `Operator / O&M Provider`
- `Technology Vendor`
- `Recruitment / Workforce Provider`
- `Other`
- `Unknown`

Populate these fields only where the project type, stage and source-supported facts reasonably support the tag. Use `Unknown` where the opportunity audience is not clear enough for filtering.
